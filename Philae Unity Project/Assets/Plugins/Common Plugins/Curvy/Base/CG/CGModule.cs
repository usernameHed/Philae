...
#pragma warning restore 618
        ...
        protected void RenameResource(string resourceName, Component resource, int index = -1)
        {
            string newName;
            {
                newName = string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0}_{1}_{2}", ModuleName, UniqueID, resourceName);
                if (index > -1)
                    newName += string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:000}", index);
            }
