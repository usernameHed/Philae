Shader ".Perso/Toon"
{
	Properties
	{
		...
		[NoScaleOffset]_EmissiveTex("Emissive Map", 2D) = "black" {}
		_EmissiveAmount("Emissive Amount", Float) = 1.0
		...
		_DissolveThickness("Dissolve Thickness", Float) = 1
		_DissolveRemapping("Dissolve Remapping", Vector) = (0,1,0,0)
	}
	SubShader
	{
		Pass
		{
			...
			float4 frag (v2f i) : SV_Target
			{
				...
				//Add emissive map
				float3 emissive = tex2D(_EmissiveTex, i.uv);
				finalColor.xyz += emissive * _EmissiveAmount;;

				//Calculate dissolve amount
				float dissolveValue = tex2D(_DissolveMap, i.uv).r;

				dissolveValue -= Remap(_DissolveAmount, float2(0, 1), _DissolveRemapping);
				finalColor += pow((1 - dissolveValue), _DissolveThickness) * _DissolveColor;
