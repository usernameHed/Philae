...
namespace hedCommon.extension.runtime.interpolation.filtering
{
    public class LoessInterpolator
    {
        ...
        {
            this._bandwidth = DEFAULT_BANDWIDTH;
        ...
        public LoessInterpolator(double bandwidth, int robustnessIters)
        {
            ...
            {
                throw new System.Exception(string.Format("bandwidth must be in the interval [0,1], but got {0}", bandwidth));
            }
            this._bandwidth = bandwidth;
            if (robustnessIters < 0)
            {
                throw new System.Exception(string.Format("the number of robustness iterations must be non-negative, but got {0}", robustnessIters));
            }
        ...
        public double[] Smooth(double[] xval, double[] yval)
        {
            ...
            {
                throw new System.Exception(string.Format("Loess expects the abscissa and ordinate arrays to be of the same size, but got {0} abscisssae and {1} ordinatae", xval.Length, yval.Length));
            }
            int n = xval.Length;
            if (n == 0)
            {
                throw new System.Exception("Loess expects at least 1 point");
            }
            ...
            {
                throw new System.Exception(string.Format("the bandwidth must be large enough to accomodate at least 2 points. There are {0} " +
            ...
            for (int iter = 0; iter <= _robustnessIters; ++iter)
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    double denom = ExtMathf.Abs(1.0f / (xval[edge] - x));
                    ...
                    res[i] = beta * x + alpha;
                    residuals[i] = ExtMathf.Abs(yval[i] - res[i]);
                }
        ...
        */
        ...
        /// </summary>
        /// <param name="values">values the values array</param>
        /// <param name="isAbscissae">if true, elements are abscissae otherwise they are ordinatae</param>
        /// <exception cref="MathException"> if one of the values is not a finite real number</exception>
        ...
        /// <exception cref="MathException">if the abscissae array is not in a strictly increasing order</exception>
